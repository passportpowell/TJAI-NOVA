...
# NEW: CLI Output Display Section
if getattr(st.session_state, 'backend_task_active', False) or captured_cli_output:
    st.subheader("📟 Live CLI Output")
    ...
    # Display all CLI output
    if st.session_state.cli_output_displayed:
        cli_text = ''.join(st.session_state.cli_output_displayed)
...
if (getattr(st.session_state, 'processing_started', False) and 
    not getattr(st.session_state, 'processing_complete', True) and 
    not getattr(st.session_state, 'param_collection_pending', False)):
    ...
    if progress_updated_in_this_run or (not os.path.exists(PROGRESS_FILE) and getattr(st.session_state, 'backend_task_active', False)):
