...
def add_object(db, strName, nClassId, bAddSystemMembership = True, strCategory = None, strDescription = None):
    return db.AddObject(strName, nClassId, bAddSystemMembership, strCategory, strDescription)
...
def add_membership(db, nCollectionId, strParent, strChild):
    return db.AddMembership(nCollectionId, strParent, strChild)
...
def get_membership_id(db, nCollectionId, strName, strChild):
    return db.GetMembershipID(nCollectionId, strName, strChild)
...
def get_objects(db, class_id):
    return db.GetObjects(nClassId = class_id)

def get_properties_table(db, collection_id, parent_name, child_name):
    return db.GetPropertiesTable(collection_id, parent_name, child_name)